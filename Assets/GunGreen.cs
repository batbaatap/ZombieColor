...
public class GunGreen : MonoBehaviour {
	...
	void Update () {
		...
		if(transform.position.x == Red.transform.position.x && gameObject.tag == "Gun"){
			print("Equal position GunBlue");
			GameObject findPoint = GameObject.FindGameObjectWithTag("findme");
			transform.position = findPoint.transform.position;					

		} else if(transform.position.x == Blue.transform.position.x && gameObject.tag == "Gun"){
